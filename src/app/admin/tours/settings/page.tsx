...
const initialDestinations = ["Cairo", "Luxor", "Aswan", "Sharm El Sheikh", "Hurghada", "Alexandria"];
const initialCategories = ["Adventure", "Relaxation", "Cultural", "Culinary", "Family", "Honeymoon", "Package", "Daily"];

function ManageListSection({ title, description, items, setItems, placeholder }: {
  title: string;
  description: string;
  items: string[];
  ...
  const handleAddItem = () => {
    if (newItem.trim() && !items.find(item => item.toLowerCase() === newItem.trim().toLowerCase())) {
        ...
        <div className="flex flex-wrap gap-2 p-4 border rounded-md min-h-24 bg-muted/50">
            {items.length > 0 ? items.map(item => (
                <Badge key={item} variant="secondary" className="text-base py-1 pl-3 pr-2">
                    {item}
                    <button onClick={() => handleRemoveItem(item)} className="ml-2 rounded-full p-0.5 hover:bg-destructive/20 text-destructive outline-none ring-offset-background focus:ring-2 focus:ring-ring focus:ring-offset-2">
                        <X className="h-3 w-3" />
                        <span className="sr-only">Remove {item}</span>
                    </button>
                </Badge>
            )) : (
                <p className="text-sm text-muted-foreground self-center mx-auto">No items yet.</p>
            )}
        </div>
      </CardContent>
...
export default function TourSettingsPage() {
  ...
  const handleSaveChanges = () => {
    ...
    alert("Changes saved to console! In a real app, this would update the database.");
  };
       ...
       <div className="flex items-center gap-4">
        ...
        <div>
            ...
            <p className="text-muted-foreground">
                Manage options for destinations and categories available when creating a new tour.
            </p>
        </div>
      </div>
