    ...
    <div className="grid gap-6 md:grid-cols-2">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Send global alerts to all agency admins.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className="space-y-4">
        ...
        {broadcasts.map((broadcast) => {
            ...
            if (broadcast.variant === 'warning') { Icon = AlertCircle; colorClass = "text-amber-500"; }
            else if (broadcast.variant === 'destructive') { Icon = XCircle; colorClass = "text-red-500"; }
            else if (broadcast.variant === 'success') { Icon = CheckCircle; colorClass = "text-green-500"; }

            return (
                <div key={broadcast.id} className="flex flex-col gap-3 p-4 rounded-lg border bg-card shadow-sm">
                    ...
                    <div className="flex items-center justify-end gap-2 pt-2 border-t mt-1">
                         <div className="flex items-center gap-1">
                             <span className={`text-[10px] uppercase font-bold ${broadcast.is_active ? "text-green-600" : "text-slate-400"}`}>
                         ...
                         <Button 
                            ...
                            disabled={pending}
                            onClick={() => startTransition(() => toggleBroadcast(broadcast.id, !broadcast.is_active))}
                         >
                             <PowerOff className={`h-4 w-4 ${broadcast.is_active ? "text-green-600" : "text-slate-400"}`} />
                         </Button>
                         <Button 
                            size="sm" 
                            variant="ghost" 
