                      ...
                      <ul className="space-y-2 text-sm">
                        {tour.includes.map((item, index) => (
                          <li key={index} className="flex items-start gap-2">
                            <span>{(() => { try { const parsed = JSON.parse(item); return parsed.value || item; } catch (e) { return item; } })()}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                ...
                <ul className="space-y-3 text-sm">
                  {tour.highlights.map((highlight, index) => (
                    <li key={index} className="flex items-start gap-3">
                      ...
                      <span className="text-muted-foreground">{(() => { try { const parsed = JSON.parse(highlight); return parsed.value || highlight; } catch (e) { return highlight; } })()}</span>
                    </li>
                  ))}
                </ul>
              </CardContent>
