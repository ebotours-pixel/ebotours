...
const generateCustomers = (): Customer[] => {
    ...
    allBookings.forEach(booking => {
        if (!customersByEmail[booking.customerEmail]) {
            customersByEmail[booking.customerEmail] = {
                ...
                totalBookings: 0,
                totalSpent: 0,
                createdAt: new Date(new Date(booking.bookingDate).getTime() - 86400000).toISOString(), // Assume they signed up a day before first booking
                bookings: [],
            };
        }
    ...
    return Object.values(customersByEmail).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
}
